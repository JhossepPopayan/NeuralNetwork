...
class NeuralNetwork:
    ...
    def inference(self,data, weights): #CALCULO DEL VALOR PREDICHO
        h11 = self.sigmoid(np.matmul(data, weights[0]))
        logits1=np.matmul(h11,weights[1])
        probs = np.exp(logits1)/np.sum(np.exp(logits1), axis=1, keepdims=True)
        return logits1
